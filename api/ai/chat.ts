    ...
    if (functionCalls && functionCalls.length > 0) {
      ...
      if (call.name === 'cancelOrder') {
        ...
        if (isCancelable) {
          ...
          // Dual-layer safety: Try to write the database update from backend
          try {
            const orderRef = doc(db, 'orders', orderId);
            ...
            console.warn(`[AI Chat API] Backend write permissions check: Relying on client-side state transition callback or pre-cancellation validation. Detail: ${writeErr.message}`);
          }

          // If BOTH server and client update has not been verified yet, we should report manual intervention
          // to comply with "do not generate success details if updateDoc fails" requirement.
          // Note that if client has pre-passed status = 'Cancelled by Customer', that is also a verified write.
